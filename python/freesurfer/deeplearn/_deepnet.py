...
class MRIDeepNet(object):
    ...
    def load_validation_image_slices(self, source_filenames, target_filenames, is_src_label_img, is_trg_label_img, orientation,):
        if self.model is None:
            raise RuntimeError('Model does not exist')
    ...
    def load_training_validation_image_slices(self, source_filename, target_filename, step_size=None):
        ...
        self.feature_generator.create_data_storage()
        self.feature_generator.generate_src_trg_slice_training_validation(source_filename, target_filename, step_size=step_size)
    ...
    def predict_labels(self, in_img_file):
        in_img = nib.load(in_img_file)
        (in_slices, in_indices) = self.feature_generator.extract_slices(in_img_file, intensity_threshold=0, is_label_img=False,indices=None)
...
class FeatureGenerator(object):
    ...
    def create_rare_training_feature_array(self, src_filenames, src_array_name, trg_filenames, trg_array_name,
                                           indices_list=None, step_size=[4,4,4], is_src_label_img=False, is_trg_label_img = True):
    ...
    def create_training_feature_array(self, image_filenames,  array_name, indices_list, step_size, is_label_img, orientation):
        nb_features_per_subject = 1000000
    ...
    def generate_src_trg_training_slice_data(self, source_filenames, target_filenames, is_src_label_img, is_trg_label_img, orientation):
        ...
        for iter_channel in range(len(self.out_channel_names)):
            target_filenames_channel = target_filenames[iter_channel]
            self.create_training_feature_array(target_filenames_channel, 'trg'+self.out_channel_names[iter_channel],
                                               indices_list, step_size=None, is_label_img=is_trg_label_img, orientation=orientation)


    def generate_src_trg_validation_slice_data(self, source_filenames, target_filenames, is_src_label_img, is_trg_label_img,orientation):
    ...
    def generate_src_trg_slice_training_validation(self, source_filename, target_filename, step_size=None):
        ...
        (val_slices, val_slice_labels, val_indices) = self.extract_training_image_slices(source_filename,
                                                                                target_filename, intensity_threshold=0,
                                                                                step_size=step_size, dataset='val')
